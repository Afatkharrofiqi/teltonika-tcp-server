...
func (r *TCPServer) Run(db *gorm.DB) error {
	logger := r.logger
...
func (r *TCPServer) handleConnection(conn net.Conn, db *gorm.DB) {
	logger := r.logger
	...
	for {
		...
		if err != nil {
			...
			for i, data := range res.Packet.Data {
				...
				if err != nil {
					logger.Error.Printf("[%s]: error marshaling elements to JSON (%v)", logKey, err)
					continue
				}
				...
				gpsData := &model.GPSData{
					...
					Elements:    string(elementsJSON), // Store JSON string in Elements field
					CreatedAt:   time.Now(),
				}
				// Save to database
